...
/// Protocol for caches that support batched operations (filter/extend)
/// Mirrors Python duck-typing for batch-capable caches
public protocol BatchableKVCache: AnyObject {
    /// In-place filter to keep just the given indices in the cache
    func filter(batchIndices: MLXArray)
}
    ...
    -> MLXFast.ScaledDotProductAttentionMaskMode
{
    ...
    // Check for batch caches FIRST, before checking t > 1 (matches Python behavior)
    if let c = cache?.first {
        // Special handling for BatchKVCache - it has its own mask logic
        if let batchCache = c as? BatchKVCache {
            return batchCache.makeMask(
                sequenceLength: t,
                returnArray: true,
                windowSize: nil
            )
        }
        // Special handling for BatchRotatingKVCache
        if let batchRotatingCache = c as? BatchRotatingKVCache {
            return batchRotatingCache.makeMask(
                sequenceLength: t,
                returnArray: true,
                windowSize: nil
            )
    ...
    // Standard logic for non-batch caches
    if t > 1 {
